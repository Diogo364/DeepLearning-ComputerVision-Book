...
def convolve(image, K):
    ...
    for y in np.arange(pad, iH + pad):
        for x in np.arange(pad, iW + pad):
            ...
            # the kernel, them summing the matrix
            k = (roi * K).sum()
...
cv2.imwrite('./outputs/11.convolutions/1.original.png', gray)
...
for kernelName, K in kernelBank:
    ...
    cv2.putText(convolveOutput, 'convolve',
        (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.9, (255, 255, 255), 2)
    
    cv2.putText(opencvOutput, 'opencv',
        (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.9, (255, 255, 255), 2)

    all_output = np.c_[convolveOutput, opencvOutput]
    cv2.imwrite(f'./outputs/11.convolutions/{kernelName}.png', all_output)
